...
def train_base(model, opt, data, scheduler, iterations, acc_steps, batch_size, sequence_length,
    ...
    using_deepspeed = hasattr(model, "backward") and hasattr(model, "step")

    device_type = 'cuda' if 'cuda' in str(extra_args.device) else 'cpu'
    type_ctx = nullcontext() if device_type == 'cpu' else torch.amp.autocast(device_type=device_type, dtype=extra_args.dtype)
    
    itr, substep, best_val_loss, text_table = srt_iter, 0, float('inf'), None
    stats = {'train_loss': [], 'val_loss': [], 'val_pp': [], 'val_acc': []}
    num_substeps_per_epoch = len(data['train']) // (batch_size * sequence_length)

    if not extra_args.no_compile and not using_deepspeed:
    ...
    while itr < iterations:
        ...
        if extra_args.grad_clip != 0.0 and not using_deepspeed:
        ...
        if itr % eval_freq == 0 or itr == iterations:
            if distributed_backend.is_master_process():
                ...
                val_acc, val_loss, val_perplexity = eval(
                    model, data['val'], sequence_length, batch_size,
        ...
        if distributed_backend.is_master_process():
            if extra_args.save_checkpoint_freq is not None and itr % extra_args.save_checkpoint_freq == 0:
                if using_deepspeed:
                    print(f"DeepSpeed saving checkpoint to {ckpt_path}")
                    model.save_checkpoint(ckpt_path, tag=f"ckpt_{itr}")
                else:
    ...
    if distributed_backend.is_master_process():
        if using_deepspeed:
            print(f"DeepSpeed saving final checkpoint to {ckpt_path}")
            model.save_checkpoint(ckpt_path, tag=f"ckpt_{itr}")
        else:
            print(f"Saving final checkpoint to {ckpt_path}/ckpt.pt")
            save_checkpoint(
                distributed_backend=distributed_backend,
                model=model,
                opt=opt,
