...
"""Full implementation of DenseFormer."""
...
class DenseFormer2(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.increase_T_every = config.increase_T_every
        assert config.n_layer % self.increase_T_every == 0
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.ModuleList([nn.ModuleList([Block(config, self.lm_cache) for _ in range(self.increase_T_every)]) for _ in range(self.n_repeat)]),
    ...
    def forward(self, idx, targets=None, get_logits=False, use_cache=False, iter=None):
        ...
        for rep_idx in range(1, self.n_repeat+1):
            ...
            print('safe_move(full_tensor, "cuda:1").device, safe_move(new_slice, "cuda:1").devic', safe_move(full_tensor, "cuda:1").device, safe_move(new_slice, "cuda:1").device)
